...
| ディレクトリ                         | 内容                                                          |
| ------------------------------------ | ------------------------------------------------------------- |
| [`website`](./website)               | 公式サイト・プライバシーポリシー（Astro / Cloudflare Worker） |
| [`connect`](./connect)               | プレイヤー接続用Cloudflare Worker                             |
...
コード整形は `mise run format`、差分を変更せず確認する場合は `mise run formatCheck` で全サブプロジェクトをまとめて実行できます。
