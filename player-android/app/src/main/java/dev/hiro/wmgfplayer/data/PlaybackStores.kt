...
class HistoryStore(context: Context) {
    ...
    suspend fun append(entry: PlaybackHistoryEntry) = withContext(Dispatchers.IO) {
        synchronized(lock) {
            val file = fileFor(entry.graphId)
            FileOutputStream(file, true).use { stream ->
                stream.write((WmgJson.format.encodeToString(entry) + "\n").toByteArray(Charsets.UTF_8))
                stream.flush()
                stream.fd.sync()
            }
    ...
    suspend fun read(graphId: String): List<PlaybackHistoryEntry> = withContext(Dispatchers.IO) {
        synchronized(lock) { readFile(fileFor(graphId)) }
    }

    suspend fun readAll(): List<PlaybackHistoryEntry> = withContext(Dispatchers.IO) {
        synchronized(lock) {
            directory.listFiles().orEmpty().flatMap(::readFile).sortedByDescending { it.endedAt }
        }
    }
    ...
    suspend fun remove(graphId: String, entryIds: Set<String>) = withContext(Dispatchers.IO) {
    ...
    suspend fun exportJsonl(): String = withContext(Dispatchers.IO) {
        ...
        entries.joinToString("\n", postfix = if (entries.isEmpty()) "" else "\n") {
            WmgJson.format.encodeToString(it)
        }
    }
    ...
    private fun readFile(file: File): List<PlaybackHistoryEntry> {
        ...
        return file.useLines(Charsets.UTF_8) { lines ->
            lines.mapNotNull { line -> runCatching { WmgJson.format.decodeFromString<PlaybackHistoryEntry>(line) }.getOrNull() }.toList()
        }
    }
    ...
    private fun rewrite(file: File, entries: List<PlaybackHistoryEntry>) {
        ...
        FileOutputStream(temporary).use { stream ->
            entries.forEach { stream.write((WmgJson.format.encodeToString(it) + "\n").toByteArray(Charsets.UTF_8)) }
            stream.flush()
            stream.fd.sync()
        }
...
class SnapshotStore(context: Context) {
    ...
    suspend fun save(snapshot: PlaybackSnapshot) = withContext(Dispatchers.IO) {
        mutex.withLock {
            ...
            try {
                stream.write(WmgJson.format.encodeToString(snapshot).toByteArray(Charsets.UTF_8))
        ...
        mutex.withLock {
            if (!file.exists()) null else runCatching {
                atomicFile.openRead().bufferedReader(Charsets.UTF_8).use { reader ->
                    WmgJson.format.decodeFromString<PlaybackSnapshot>(reader.readText())
                }
            }.getOrNull()
        }
    }
