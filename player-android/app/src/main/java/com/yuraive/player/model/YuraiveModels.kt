...
@Serializable
data class YuraiveGraph(
    val version: Int,
    val metadata: YuraiveMetadata? = null,
    val nodes: Map<String, YuraiveNode>,
    val buttons: Map<String, YuraiveButton>,
...
object GraphValidator {
    fun validate(graph: YuraiveGraph): List<ValidationIssue> =
        validateJson(YuraiveJson.format.encodeToString(YuraiveGraph.serializer(), graph))

    fun validateJson(json: String): List<ValidationIssue> = NativeGraphValidator.validate(json)

    fun allAssetPaths(graph: YuraiveGraph): Set<String> = buildSet {
