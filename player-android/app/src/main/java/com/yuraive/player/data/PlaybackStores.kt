...
class HistoryStore(context: Context) {
    ...
    suspend fun append(entry: PlaybackHistoryEntry) = withContext(Dispatchers.IO) {
        synchronized(lock) {
            ...
            FileOutputStream(file, true).use { stream ->
                stream.write((YuraiveJson.format.encodeToString(entry) + "\n").toByteArray(Charsets.UTF_8))
                stream.flush()
                stream.fd.sync()
            }
    ...
    private fun readFile(file: File): List<PlaybackHistoryEntry> {
        ...
        return file.useLines(Charsets.UTF_8) { lines ->
            lines.mapNotNull { line -> runCatching { YuraiveJson.format.decodeFromString<PlaybackHistoryEntry>(line) }.getOrNull() }.toList()
        }
    }
    ...
    private fun rewrite(file: File, entries: List<PlaybackHistoryEntry>) {
        ...
        FileOutputStream(temporary).use { stream ->
            entries.forEach { stream.write((YuraiveJson.format.encodeToString(it) + "\n").toByteArray(Charsets.UTF_8)) }
            stream.flush()
            stream.fd.sync()
        }
...
class SnapshotStore(context: Context) {
    ...
    suspend fun save(snapshot: PlaybackSnapshot) = withContext(Dispatchers.IO) {
        mutex.withLock {
            ...
            try {
                stream.write(YuraiveJson.format.encodeToString(snapshot).toByteArray(Charsets.UTF_8))
        ...
        mutex.withLock {
            if (!file.exists()) null else runCatching {
                atomicFile.openRead().bufferedReader(Charsets.UTF_8).use { reader ->
                    YuraiveJson.format.decodeFromString<PlaybackSnapshot>(reader.readText())
                }
            }.getOrNull()
        }
    }
