...
object PlaybackRuntime {
    ...
    internal fun publish(value: PlaybackUiState) {
    ...
    internal fun attachPlayer(value: Player?) {
        mutablePlayer.value = value
    }
    ...
    fun initialize(context: Context) =
        send(
            ...
            Intent(context, PlaybackService::class.java)
                .setAction(PlaybackService.ACTION_INITIALIZE),
            foreground = false,
        )

    fun play(context: Context, ref: GraphRef) =
        send(
            context,
            Intent(context, PlaybackService::class.java)
    ...
    fun pressButton(context: Context, buttonId: String) =
        send(
            context,
            Intent(context, PlaybackService::class.java)
                .setAction(PlaybackService.ACTION_BUTTON)
                .putExtra(PlaybackService.EXTRA_BUTTON_ID, buttonId),
            true,
        )
    ...
    fun restart(context: Context) =
        send(
            context,
            Intent(context, PlaybackService::class.java).setAction(PlaybackService.ACTION_RESTART),
            true,
        )
    ...
    fun previous(context: Context) =
        send(
            context,
            Intent(context, PlaybackService::class.java).setAction(PlaybackService.ACTION_PREVIOUS),
            true,
        )
    ...
    private fun send(
        context: Context,
        intent: Intent,
        @Suppress("UNUSED_PARAMETER") foreground: Boolean,
    ) {
