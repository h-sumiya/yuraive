...
enum class RemoteProtocol { SMB, WEBDAV }
...
internal object RemotePaths {
    ...
    fun validate(config: RemoteConnectionConfig): String? = when (config.protocol) {
        RemoteProtocol.SMB -> when {
            ...
            config.share.contains('/') || config.share.contains('\\') -> "共有名に / や \\ は使用できません"
            config.share.trim() == "." || config.share.trim() == ".." -> "共有名が正しくありません"
            else -> null
        }
        RemoteProtocol.WEBDAV -> runCatching {
            ...
            when {
                ...
                uri.query != null || uri.fragment != null -> "WebDAV URL にクエリやフラグメントは使用できません"
                else -> null
            }
    ...
    fun normalizeRelative(path: String): String {
        ...
        require(value.split('/').none { it == "." || it.any(Char::isISOControl) }) { "安全でないフォルダパスです" }
        require(GraphValidator.isSafeRelativePath(value)) { "安全でないフォルダパスです" }
        return value
    }

    fun join(left: String, right: String): String = listOf(left.trim('/'), right.trim('/'))
        .filter(String::isNotEmpty)
        .joinToString("/")

    fun normalizedEndpoint(value: String): String = value.trim().trimEnd('/') + "/"

    fun isSafeSegment(value: String): Boolean = value.isNotBlank() && value != "." && value != ".." &&
        value.none { it == '/' || it == '\\' || it == ':' || it == '\u0000' || it.isISOControl() }
}
...
internal class RemoteSourceManager(private val context: Context) {
    ...
    suspend fun browse(config: RemoteConnectionConfig, relativePath: String): List<RemoteFolder> = withContext(Dispatchers.IO) {
        ...
        backend(config.copy(rootPath = "")).list(normalized)
            .asSequence()
    ...
    fun save(config: RemoteConnectionConfig, selectedPath: String, fallbackName: String): RootGrant {
        RemotePaths.validate(config)?.let(::error)
        ...
        val name = config.displayName.trim().ifBlank { fallbackName.ifBlank { defaultName(config, rootPath) } }
        val saved = config.copy(
            id = id,
            displayName = name,
            endpoint = if (config.protocol == RemoteProtocol.WEBDAV) RemotePaths.normalizedEndpoint(config.endpoint) else "",
    ...
    fun mediaUri(rootUri: String, relativePath: String): Uri {
        ...
        RemotePaths.normalizeRelative(relativePath).split('/').filter(String::isNotEmpty).forEach(builder::appendPath)
    ...
    fun materialize(rootUri: String, relativePath: String, modifiedAt: Long, size: Long): Uri {
        ...
        val suffix = path.substringAfterLast('.').takeIf { it.length in 1..8 && it.all(Char::isLetterOrDigit) }?.let { ".$it" }.orEmpty()
        ...
        if (!target.isFile) {
            ...
            try {
                open(rootUri, path).use { read ->
                    require(read.totalLength <= MAX_MATERIALIZED_BYTES || read.totalLength < 0) { "画像ファイルが大きすぎます" }
    ...
    private fun configById(id: String): RemoteConnectionConfig = configs[id] ?: store.get(id)?.also { configs[id] = it }
        ?: error("接続情報が見つかりません。フォルダを追加し直してください")

    private fun backend(config: RemoteConnectionConfig): RemoteBackend = when (config.protocol) {
        RemoteProtocol.SMB -> SmbBackend(config)
        RemoteProtocol.WEBDAV -> WebDavBackend(config, httpClient)
    }

    private fun rootUri(protocol: RemoteProtocol, id: String): String = when (protocol) {
    ...
    private fun defaultName(config: RemoteConnectionConfig, path: String): String = path.substringAfterLast('/').ifBlank {
        if (config.protocol == RemoteProtocol.SMB) config.share else URI(config.endpoint).host.orEmpty()
    }
...
private class SmbBackend(private val config: RemoteConnectionConfig) : RemoteBackend {
    override fun list(relativePath: String): List<RemoteNode> = try {
    ...
    private fun connect(): SmbResources {
        ...
        try {
            ...
            val authentication = if (config.username.isBlank() && config.password.isBlank()) {
    ...
    private fun smbPath(path: String): String = RemotePaths.normalizeRelative(path).replace('/', '\\')
...
private class ResourceInputStream(input: InputStream, private val resources: Closeable) : FilterInputStream(input) {
    override fun close() {
        try {
            super.close()
        } finally {
...
internal class WebDavBackend(
    ...
    override fun list(relativePath: String): List<RemoteNode> {
        ...
        val request = requestBuilder(url(requested, directory = true))
            .header("Depth", "1")
    ...
    override fun open(relativePath: String, offset: Long): RemoteRead {
        val request = requestBuilder(url(RemotePaths.normalizeRelative(relativePath), directory = false))
        ...
        try {
            ...
            if (response.code == 206 && rangeStart != offset) throw IOException("WebDAVサーバーが異なるRangeを返しました")
            val rangeTotal = contentRange?.substringAfterLast('/', "")?.toLongOrNull()
            val totalLength = rangeTotal ?: when {
            ...
            input = object : FilterInputStream(input) {
                override fun close() {
                    try {
                        super.close()
                    } finally {
    ...
    private fun url(relativePath: String, directory: Boolean): String {
        val encoded = RemotePaths.normalizeRelative(relativePath)
            .split('/')
            .filter(String::isNotEmpty)
    ...
    private fun parseListing(bytes: ByteArray, requestedPath: String): List<RemoteNode> {
        val asciiProjection = buildString(bytes.size) {
            bytes.forEach { byte ->
        ...
        for (index in 0 until responseNodes.length) {
            ...
            val href = response.getElementsByTagNameNS("DAV:", "href").item(0)?.textContent?.trim().orEmpty()
            if (href.isEmpty()) continue
            val absolute = runCatching { requestUri.resolve(href) }.getOrNull() ?: continue
            if (absolute.host != null && !absolute.host.equals(URI(endpoint).host, ignoreCase = true)) continue
            val segments = runCatching { decodedPathSegments(absolute.rawPath) }.getOrNull() ?: continue
            if (segments.take(endpointSegments.size) != endpointSegments) continue
            val relative = segments.drop(endpointSegments.size)
            if (relative.size != requestedSegments.size + 1 || relative.take(requestedSegments.size) != requestedSegments) continue
            ...
            val size = properties.getElementsByTagNameNS("DAV:", "getcontentlength").item(0)?.textContent?.trim()?.toLongOrNull() ?: 0
            val modified = properties.getElementsByTagNameNS("DAV:", "getlastmodified").item(0)?.textContent?.trim()?.let {
                runCatching { ZonedDateTime.parse(it, DateTimeFormatter.RFC_1123_DATE_TIME).toInstant().toEpochMilli() }.getOrDefault(0)
            } ?: 0
            result[name] = RemoteNode(name, isDirectory, size.coerceAtLeast(0), modified.coerceAtLeast(0))
        }
...
<d:propfind xmlns:d="DAV:"><d:prop><d:resourcetype/><d:getcontentlength/><d:getlastmodified/></d:prop></d:propfind>"""
        ...
        private fun decodedPathSegments(rawPath: String?): List<String> = rawPath.orEmpty().split('/')
            .filter(String::isNotEmpty)
            .map { segment ->
                URLDecoder.decode(segment.replace("+", "%2B"), StandardCharsets.UTF_8.name())
            }
            .also { segments -> require(segments.none { it == "." || it == ".." || it.contains('/') || it.contains('\\') }) }
        ...
        private fun readLimited(input: InputStream, maxBytes: Int): ByteArray = input.use {
...
private class EncryptedRemoteConfigStore(context: Context) {
    ...
    fun get(id: String): RemoteConnectionConfig? = preferences.getString(id, null)?.let { encoded ->
        runCatching {
            ...
            YuraiveJson.format.decodeFromString<RemoteConnectionConfig>(plaintext.decodeToString())
