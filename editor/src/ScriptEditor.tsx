...
const completions = [
  ...
  { label: 'ctx["history"][0]["schemaVersion"]', type: 'property', detail: '履歴スキーマバージョン' },
  ...
  { label: 'ctx["history"][0]["mediaDurationMs"]', type: 'property', detail: '履歴のメディア尺 (ms)' },
  { label: 'ctx["history"][0]["activePlayMs"]', type: 'property', detail: '履歴の実再生時間 (ms)' },
  { label: 'ctx["history"][0]["startPositionMs"]', type: 'property', detail: '履歴の開始位置 (ms)' },
  ...
  { label: 'True', type: 'keyword' }, { label: 'False', type: 'keyword' }, { label: 'None', type: 'keyword' },
]
...
const editorTheme = EditorView.theme({
  ...
  '.cm-content': { fontFamily: '"SFMono-Regular", Consolas, "Liberation Mono", monospace', padding: '12px 0', caretColor: '#70c7ef' },
  '.cm-line': { padding: '0 18px' },
  '.cm-cursor, .cm-dropCursor': { borderLeftColor: '#70c7ef' },
  '&.cm-focused .cm-selectionBackground, .cm-selectionBackground, .cm-content ::selection': { backgroundColor: '#24465c' },
  ...
  '.cm-tooltip-autocomplete > ul > li[aria-selected]': { backgroundColor: '#24516b', color: '#fff' },
}, { dark: true })
...
export function ScriptEditor({ script, test, onChange, onSave, onTest, statsSessions = [] }: {
  ...
  useEffect(() => {
    ...
    const state = EditorState.create({
      ...
      extensions: [
        lineNumbers(), highlightActiveLineGutter(), foldGutter(), history(), drawSelection(), dropCursor(), EditorState.allowMultipleSelections.of(true),
        indentOnInput(), bracketMatching(), closeBrackets(), rectangularSelection(), highlightActiveLine(), python(), syntaxHighlighting(defaultHighlightStyle, { fallback: true }),
  ...
  useEffect(() => {
    ...
    if (!editor) return
    const diagnostics: Diagnostic[] = test.status === 'error' ? [{
      from: test.line ? editor.state.doc.line(Math.min(test.line, editor.state.doc.lines)).from + Math.max(0, (test.column ?? 1) - 1) : 0,
      to: test.line ? editor.state.doc.line(Math.min(test.line, editor.state.doc.lines)).to : Math.min(1, editor.state.doc.length),
      severity: 'error',
  ...
  return <section className="script-editor-pane">
    ...
    <footer className={`script-status ${test.status}`}>
      <div>{test.status === 'error' ? <><strong>エラー</strong><span>{test.message}</span></> : test.status === 'success' ? <><strong>成功</strong><span>{test.durationMs?.toFixed(1)}ms · {JSON.stringify(test.result)}</span></> : <span>Starlark · UTF-8 · Space: 4</span>}</div>
      <div><span>{functions.length} functions</span><span>Ln {cursor.line}, Col {cursor.column}</span></div>
    </footer>
  </section>
}
