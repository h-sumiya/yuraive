...
const leafName = (path: string) => path.includes('/') ? path.slice(path.lastIndexOf('/') + 1) : path
...
const mimeType = (path: string) => {
  ...
  return ({
    ...
    json: 'application/json', star: 'text/plain', txt: 'text/plain', srt: 'text/plain', vtt: 'text/vtt',
...
const request = (action: string, values: Record<string, unknown>) => {
  ...
  const promise = new Promise<void>((resolve, reject) => pendingRequests.set(id, { resolve, reject }))
...
const createBridgeDirectory = (message: NativeDirectoryMessage) => {
  ...
  const createNativeFile = (path: string) => {
    ...
    if (!entry || entry.kind !== 'file') throw new DOMException(`${path} が見つかりません`, 'NotFoundError')
    const url = `${message.contentBaseUrl}${encodePath(path)}?v=${entry.lastModified ?? 0}`
    const file = new File([], leafName(path), { type: mimeType(path), lastModified: entry.lastModified }) as NativeFile
  ...
  const createFileHandle = (path: string) => ({
    ...
    createWritable: async () => {
      ...
      return {
        write: async (value: FileSystemWriteChunkType) => { content = value },
        close: async () => {
          const nativeSource = content instanceof File ? (content as NativeFile).__yuraiveNativePath : undefined
          if (nativeSource) {
            ...
            else if (ArrayBuffer.isView(content)) buffer = content.buffer.slice(content.byteOffset, content.byteOffset + content.byteLength) as ArrayBuffer
            else throw new Error('この書き込み形式には対応していません')
            await request('write', { path, data: bytesToBase64(buffer) })
            const current = entries.get(path)
            if (current) entries.set(path, { ...current, size: buffer.byteLength, lastModified: Date.now() })
          }
        },
        abort: async () => undefined,
        seek: async () => { throw new Error('部分書き込みには対応していません') },
        truncate: async () => { throw new Error('部分書き込みには対応していません') },
  ...
  const createDirectoryHandle = (path = ''): FileSystemDirectoryHandle => ({
    ...
    entries: async function* () {
      ...
      for (const child of children) {
        yield [leafName(child.path), child.kind === 'directory' ? createDirectoryHandle(child.path) : createFileHandle(child.path)] as [string, FileSystemDirectoryHandle | FileSystemFileHandle]
      }
    },
    getDirectoryHandle: async (name: string, options?: FileSystemGetDirectoryOptions) => {
      ...
      if (!current) {
        if (!options?.create) throw new DOMException(`${childPath} が見つかりません`, 'NotFoundError')
        await request('ensure-directory', { path: childPath })
        entries.set(childPath, { path: childPath, kind: 'directory' })
      } else if (current.kind !== 'directory') throw new DOMException(`${childPath} はフォルダではありません`, 'TypeMismatchError')
    ...
    getFileHandle: async (name: string, options?: FileSystemGetFileOptions) => {
      ...
      if (!current) {
        ...
        entries.set(childPath, { path: childPath, kind: 'file', size: 0, lastModified: Date.now() })
      } else if (current.kind !== 'file') throw new DOMException(`${childPath} はファイルではありません`, 'TypeMismatchError')
    ...
    removeEntry: async (name: string, options?: FileSystemRemoveOptions) => {
      ...
      for (const candidate of [...entries.keys()]) {
        if (candidate === childPath || candidate.startsWith(`${childPath}/`)) entries.delete(candidate)
      }
    },
...
const receiveNativeMessage = (event: WebViewMessageEvent) => {
  ...
  if (transferredHandle && typeof transferredHandle === 'object' && (transferredHandle as FileSystemHandle).kind === 'directory') {
...
export const requestNativeDirectory = () => {
  ...
  if (!nativeDirectoryPromise) nativeDirectoryPromise = new Promise((resolve) => { resolveNativeDirectory = resolve })
