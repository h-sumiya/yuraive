...
export const LAYOUT_CSS_PROPERTIES = [
  ...
  'gap', 'row-gap', 'column-gap', 'place-items', 'place-content', 'place-self', 'justify-items', 'justify-content', 'justify-self',
  'align-items', 'align-content', 'align-self', 'order', 'position', 'inset', 'top', 'right', 'bottom', 'left', 'z-index',
  'width', 'height', 'min-width', 'min-height', 'max-width', 'max-height', 'padding', 'margin', 'overflow', 'box-sizing',
  'background', 'background-color', 'background-image', 'background-size', 'background-position', 'color', 'opacity', 'border',
  'border-color', 'border-width', 'border-radius', 'box-shadow', 'font', 'font-family', 'font-size', 'font-weight', 'line-height',
  'letter-spacing', 'text-align', 'white-space', 'text-overflow', 'transform', 'filter', 'pointer-events', 'container', 'container-type', 'container-name',
] as const
...
const sanitizedLayoutParts = (source: string) => {
  ...
  const styles = [...parsed.querySelectorAll('style')].map((style) => sanitizeLayoutCss(style.textContent ?? ''))
...
const sanitizeLayoutCss = (source: string) => source
  .replace(/@import\s+(?:url\([^)]*\)|[^;]+);?/gi, '')
  .replace(/url\(\s*(['"]?)(.*?)\1\s*\)/gi, (value, _quote: string, url: string) => /^(?:data:|blob:|content:)/i.test(url.trim()) ? value : 'none')

export const layoutSlotNames = (source: string): string[] => {
  const parsed = new DOMParser().parseFromString(source, 'text/html')
  return [...parsed.querySelectorAll('slot')].map((slot) => (slot.getAttribute('name') || slot.id).trim())
}
...
export const validateLayoutSource = (source: string): LayoutSourceIssue[] => {
  ...
  if (!slots.some((slot) => !(slot.getAttribute('name') || slot.id).trim())) issues.push({ severity: 'error', message: 'name/idのないデフォルトslotが必要です' })
  const names = slots.map((slot) => (slot.getAttribute('name') || slot.id).trim())
  const duplicates = [...new Set(names.filter((name, index) => names.indexOf(name) !== index))]
  if (duplicates.length) issues.push({ severity: 'error', message: `slot名が重複しています: ${duplicates.map((name) => name || '(default)').join(', ')}` })
  for (const element of [...parsed.body.querySelectorAll('*')]) {
    if (!allowedElements.has(element.tagName)) issues.push({ severity: 'warning', message: `<${element.tagName.toLowerCase()}>は除去されます。divとslotのみ使用できます` })
    for (const attribute of [...element.attributes]) {
      if (!allowedAttributes.has(attribute.name)) issues.push({ severity: 'warning', message: `${element.tagName.toLowerCase()}の${attribute.name}属性は除去されます` })
    }
  }
  if (!parsed.querySelector('style')) issues.push({ severity: 'warning', message: 'style要素がありません。ボタンには暗黙の外観が適用されません' })
  return issues
}
...
const applyButtonStyle = (element: HTMLButtonElement, style: ButtonRenderStyle | undefined, backgroundImageUrl?: string) => {
  if (!style) return
  ...
  if (style.borderWidth !== undefined) { element.style.borderWidth = `${style.borderWidth}px`; element.style.borderStyle = 'solid' }
  ...
  if (style.paddingHorizontal !== undefined) { element.style.paddingLeft = `${style.paddingHorizontal}px`; element.style.paddingRight = `${style.paddingHorizontal}px` }
  if (style.paddingVertical !== undefined) { element.style.paddingTop = `${style.paddingVertical}px`; element.style.paddingBottom = `${style.paddingVertical}px` }
}
...
export const syncLayoutRoot = (root: ShadowRoot, buttons: LayoutButton[], onPress?: (id: string) => void) => {
  ...
  buttons.filter((button) => button.visible).sort((left, right) => (left.order ?? 0) - (right.order ?? 0)).forEach((button) => {
