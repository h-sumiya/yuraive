...
const PreviewIcon = ({
  ...
  name: 'play' | 'pause' | 'close' | 'debug' | 'trash' | 'export'
}) => {
  const path =
    name === 'play'
      ? 'm7 4 13 8-13 8z'
      : name === 'pause'
        ...
        : name === 'close'
          ...
          : name === 'trash'
            ? 'M4 7h16M9 3h6l1 4H8zM6 7l1 14h10l1-14'
            : name === 'export'
              ? 'M12 3v12m-4-4 4 4 4-4M4 17v4h16v-4'
              : 'M8 9h8M9 4h6l1 3H8zM7 7l-2 3v8l3 3h8l3-3v-8l-2-3M3 13h4m10 0h4'
  return (
    <svg className="icon" width="15" height="15" viewBox="0 0 24 24" aria-hidden="true">
      <path d={path} />
    </svg>
  )
}
...
function useObjectUrl(file?: File) {
  ...
  useEffect(() => {
    ...
    setUrl(next)
    return () => {
      window.setTimeout(() => URL.revokeObjectURL(next), 1000)
    }
  }, [file])
  return url
}
...
const safeRenderResult = (value: unknown): ButtonRenderResult => {
  ...
  if (raw.style && typeof raw.style === 'object' && !Array.isArray(raw.style)) {
    ...
    result.style = {}
    for (const key of ['backgroundColor', 'backgroundImage', 'textColor', 'borderColor'] as const)
      if (typeof style[key] === 'string') result.style[key] = String(style[key]).slice(0, 500)
    for (const key of [
      'opacity',
      'borderWidth',
      'borderRadius',
...
export function Preview({
  ...
  initialHistory?: PlaybackHistoryEntry[]
  onHistoryChange?: (history: PlaybackHistoryEntry[]) => void
  onClose: () => void
}) {
  const start =
    Object.entries(graph.nodes).find(([, node]) => node.start)?.[0] ?? Object.keys(graph.nodes)[0]
  ...
  const asset = useCallback(
    (path?: string) => assets.find((item) => item.path === path)?.file,
    [assets],
  )
  ...
  const mediaFile =
    candidate?.source.type === 'video'
      ? asset(candidate.source.video)
      : asset(candidate?.source.audio)
  const imageFile =
    candidate?.source.type === 'audioImage' ? asset(candidate.source.image) : undefined
  ...
  const addTrace = useCallback(
    (kind: PreviewTraceEntry['kind'], title: string, detail?: string, data?: unknown) => {
      if (!mounted.current) return
      setTrace((items) => [
        ...items.slice(-499),
        { id: crypto.randomUUID(), at: new Date().toISOString(), kind, title, detail, data },
      ])
    },
    [],
  )
  ...
  const currentContext = useCallback((snapshot: PlaybackHistoryEntry[]): StarlarkCurrent | null => {
    ...
    if (
      active &&
      active.nodeId === selected.nodeId &&
      active.candidate.id === selected.candidate?.id
    ) {
      const runningMs =
        active.playingSince === undefined ? 0 : performance.now() - active.playingSince
      return {
    ...
    const completed =
      finalizedCurrent.current?.nodeId === selected.nodeId &&
      finalizedCurrent.current.mediaId === selected.candidate?.id
        ? finalizedCurrent.current
        : snapshot.findLast(
            (entry) => entry.nodeId === selected.nodeId && entry.mediaId === selected.candidate?.id,
          )
    if (completed)
      return {
    ...
    return {
      nodeId: selected.nodeId,
      mediaId: selected.candidate?.id ?? null,
      source: selected.candidate?.source.video ?? selected.candidate?.source.audio ?? null,
      startedAt: null,
      positionMs: 0,
      mediaDurationMs: 0,
      activePlayMs: 0,
  ...
  const scriptContext = useCallback(
    (snapshot: PlaybackHistoryEntry[], trigger: Record<string, unknown>) => {
      const selected = currentRef.current
      const current = currentContext(snapshot)
      const currentIsFinalized =
        !tracker.current &&
        Boolean(
          selected?.candidate &&
          finalizedCurrent.current?.nodeId === selected.nodeId &&
  ...
  const finalize = useCallback(
    (reason: PlaybackHistoryEntry['endReason']) => {
      const active = tracker.current
      ...
      addTrace(
        'history',
        `${entry.mediaId} を履歴へ追加`,
        `${entry.activePlayMs}ms · ${reason}`,
        entry,
      )
      return next
    },
  ...
  const resolveToMedia = useCallback(
    async (
      firstNodeId: string,
      trigger: Record<string, unknown>,
      snapshot = historyRef.current,
    ) => {
      setResolving(true)
      ...
      for (let hop = 0; hop < 32; hop++) {
        const target = graph.nodes[nodeId]
        if (!target) {
          addTrace('error', `遷移先がありません: ${nodeId}`)
          setResolving(false)
          return
        }
        ...
        addTrace(
          'script',
          `Script Node実行: ${nodeId}`,
          call ? `${call.path}#${call.function ?? 'jump'}` : '未設定',
        )
        ...
        if (call?.path) {
          try {
            ...
            const result = await runStarlark({
              scripts,
              path: call.path,
              functionName: call.function ?? 'jump',
              args: [context],
              timeoutMs: 1200,
            })
            ...
            if (typeof result.value === 'string') nextId = result.value
              ...
              throw new Error('jump()はNode IDの文字列またはNoneを返してください')
            addTrace(
              'script',
              `戻り値: ${nextId ?? 'None'}`,
        ...
        if (!nextId) {
          addTrace('error', `Script Node ${nodeId} から遷移できません`)
          setResolving(false)
          return
        }
        nodeId = nextId
  ...
  const transitionFromCurrent = useCallback(
    async (
      reason: PlaybackHistoryEntry['endReason'],
      trigger: Record<string, unknown>,
  ...
  useEffect(() => {
    ...
    if (!started.current) {
      ...
      void resolveToMedia(start, { type: 'start' }, [])
    }
    return () => {
  ...
  useEffect(() => {
    const paths = new Set(
      Object.entries(graph.buttons)
        .flatMap(([id, button]) => [
          button.style?.backgroundImage,
          buttonResults[id]?.style?.backgroundImage,
        ])
        .filter(Boolean) as string[],
    )
    const next = Object.fromEntries(
      [...paths].flatMap((path) => {
        const file = assets.find((item) => item.path === path)?.file
        return file ? [[path, nativeFileUrl(file) ?? URL.createObjectURL(file)]] : []
      }),
    )
  ...
  const evaluateButtons = useCallback(async () => {
    ...
    await Promise.all(
      (node.buttons ?? []).map(async (buttonId) => {
        ...
        try {
          ...
          const result = await runStarlark({
            scripts,
            path: button.render.path,
            functionName: button.render.function ?? 'render',
            args: [context],
            timeoutMs: 1200,
          })
  ...
  }, [current?.nodeId, evaluateButtons, history.length])

  const exportHistory = () => {
    const content =
      historyRef.current.map((entry) => JSON.stringify(entry)).join('\n') +
      (historyRef.current.length ? '\n' : '')
    const url = URL.createObjectURL(new Blob([content], { type: 'application/x-ndjson' }))
    const anchor = document.createElement('a')
    anchor.href = url
    anchor.download = `${graphId.replace(/\.yuraive\.json$/i, '')}-preview-history.jsonl`
    anchor.click()
  ...
  const close = () => {
    finalize('stopped')
    resetStarlarkRuntime()
    onClose()
  }
  const restart = async () => {
    const snapshot = finalize('restarted')
    runId.current = crypto.randomUUID()
    runStartedAt.current = new Date().toISOString()
    await resolveToMedia(start, { type: 'restart' }, snapshot)
  }
  const next = async () => {
    await transitionFromCurrent('completed', { type: 'next' })
  }
  const toggleMedia = () => {
  ...
  const layoutButtons =
    !resolving && node?.type === 'media'
      ? (node.buttons ?? []).flatMap((buttonId) => {
          const button = graph.buttons[buttonId]
          if (!button) return []
          const rendered = buttonResults[buttonId] ?? {}
          const withinTime =
            !button.visibility?.length ||
            button.visibility.some(
              (interval) =>
                positionMs >= interval.fromMs &&
                (interval.toMs === null || positionMs <= interval.toMs),
            )
      ...
      <div className={`preview-modal ${debugOpen ? 'debug-open' : ''}`}>
        <header>
          <div>
            ...
            <span>
              {node
                ? `${node.editor?.label || current?.nodeId} - ${current?.nodeId}`
                : resolving
                  ? '遷移を解決中…'
                  : '停止'}
            </span>
          ...
          <div>
            <button
              className={`icon-button ${debugOpen ? 'active' : ''}`}
              title="デバッグパネル"
              onClick={() => setDebugOpen(!debugOpen)}
            >
              <PreviewIcon name="debug" />
            </button>
        ...
        <div className="preview-body">
          <div className="preview-stage">
            {imageUrl && (
              <>
                <img className="preview-artwork-cover" src={imageUrl} aria-hidden="true" alt="" />
                <span className="preview-artwork-shade" />
                <img className="preview-artwork-contain" src={imageUrl} alt="" />
              </>
            )}
            {candidate?.source.type === 'video' && mediaUrl && (
              <video
                ref={(element) => {
                ...
                onLoadedMetadata={(event) => {
                  const value = Number.isFinite(event.currentTarget.duration)
                    ...
                    : 0
                  if (tracker.current) tracker.current.durationMs = value
                  setDurationMs(value)
                }}
                onPlay={() => {
                  setPlaying(true)
                  if (tracker.current && tracker.current.playingSince === undefined)
                    tracker.current.playingSince = performance.now()
                }}
                onPause={() => {
                  setPlaying(false)
                  const item = tracker.current
                  if (item?.playingSince !== undefined) {
                    item.activePlayMs += performance.now() - item.playingSince
                ...
                onEnded={() => void transitionFromCurrent('completed', { type: 'end' })}
              />
            )}
            {candidate?.source.type !== 'video' && mediaUrl && (
              <audio
            ...
            {!candidate && (
              <div className="preview-empty">
                ...
                {!resolving && node && !node.terminal && node.onEnd?.length ? (
                  <button
                    className="primary-button"
                    onClick={() => void transitionFromCurrent('completed', { type: 'empty' })}
                  >
                    即時遷移を実行
                  </button>
                ) : null}
            ...
            {layoutSource && (
              <LayoutFrame
                ...
                onPress={(buttonId) => {
                  const button = graph.buttons[buttonId]
                  if (button) void onButton(buttonId, button)
                }}
              />
            )}
            {!layoutSource && Boolean(node?.buttons?.length) && (
              <div className="preview-layout-missing">レイアウトファイルが接続されていません</div>
            )}
          </div>
          {debugOpen && (
            <aside className="preview-debug">
              <header>
                <div>
                  <button
                    className={debugTab === 'trace' ? 'active' : ''}
                    onClick={() => setDebugTab('trace')}
                  >
                    Trace <i>{trace.length}</i>
                  </button>
                  <button
                    className={debugTab === 'history' ? 'active' : ''}
                    onClick={() => setDebugTab('history')}
                  >
                    History <i>{history.length}</i>
                  </button>
                  <button
                    className={debugTab === 'context' ? 'active' : ''}
              ...
              <div className="preview-debug-content">
                {debugTab === 'trace' &&
                  (trace.length ? (
                    [...trace].reverse().map((entry) => (
                      <details className={`trace-row ${entry.kind}`} key={entry.id}>
                        <summary>
                    ...
                    <div className="debug-empty">実行ログはまだありません</div>
                  ))}
                {debugTab === 'history' &&
                  (history.length ? (
                    [...history].reverse().map((entry) => (
                      <details className="history-row" key={entry.id}>
                        <summary>
                          <strong>{entry.mediaId}</strong>
                          <span>{(entry.activePlayMs / 1000).toFixed(1)}秒</span>
                          <small>{entry.endReason}</small>
                        </summary>
                        <pre>{JSON.stringify(entry, null, 2)}</pre>
                      </details>
                    ))
                  ) : (
                    <div className="debug-empty">プレビュー中の履歴はまだありません</div>
                  ))}
                {debugTab === 'context' && (
                  <pre className="context-json">{JSON.stringify(contextPreview, null, 2)}</pre>
                )}
              </div>
              <footer>
                <span>{resolving ? '● resolving' : '● ready'}</span>
                <span>{scripts.length} scripts · max 32 hops</span>
              </footer>
            </aside>
          )}
        </div>
        <div className="preview-transport">
          <button
            ...
            onClick={toggleMedia}
          >
            <PreviewIcon name={playing ? 'pause' : 'play'} />
          </button>
          ...
          <input
            ...
            disabled={!mediaUrl || durationMs <= 0}
            onChange={(event) => seek(Number(event.currentTarget.value))}
          />
          <span>{formatPreviewTime(durationMs)}</span>
        </div>
        <footer>
          <span>履歴はこのプレビュー内のメモリにのみ保持されます</span>
          <div>
            <button className="text-button" onClick={() => void evaluateButtons()}>
              表示Scriptを再実行
            </button>
            ...
            <button
              ...
              disabled={resolving || !node?.onEnd?.length}
              onClick={() => void next()}
            >
              次へ
            </button>
