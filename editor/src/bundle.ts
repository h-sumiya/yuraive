...
const stripEditorState = (graph: YuraiveGraph): YuraiveGraph =>
  JSON.parse(
    JSON.stringify(graph, (key, value) => (key === 'editor' ? undefined : value)),
  ) as YuraiveGraph

const textFilesForGraph = (
  graphPath: string,
  ...
  const relative = (path: string) =>
    parent && path.startsWith(parent) ? path.slice(parent.length) : path
  return [
    ...scripts
      .filter((script) => !parent || script.path.startsWith(parent))
      .map((script) => ({
    ...
    ...layouts
      .filter((layout) => !parent || layout.path.startsWith(parent))
      .map((layout) => ({
...
export const playerBundleName = (jsonName: string) =>
  jsonName.replace(/\.yuraive\.json$/i, '') + '.yuraive'

export const createPlayerBundle = (
  graphPath: string,
  graph: YuraiveGraph,
  scripts: ScriptDocument[],
  layouts: LayoutDocument[],
  ...
  if (graphBytes.length > MAX_GRAPH_SIZE)
    throw new Error('グラフがバンドル上限の8 MiBを超えています')

  const files = textFilesForGraph(graphPath, scripts, layouts)
  if (files.length > MAX_TEXT_FILES)
    throw new Error(`テキストファイルが多すぎます（上限${MAX_TEXT_FILES}件）`)
  ...
  const requiredLayouts = Object.values(graph.playerControls)
    .map((control) => control.layout)
    .filter((path): path is string => Boolean(path))
  requiredScripts.forEach((path) => {
    if (!availableScripts.has(path))
      throw new Error(`コンテンツルート内にスクリプトが見つかりません: ${path}`)
  })
  requiredLayouts.forEach((path) => {
    if (!availableLayouts.has(path))
      throw new Error(`コンテンツルート内にレイアウトが見つかりません: ${path}`)
  })
  const seen = new Set<string>()
  let textTotalSize = 0
  const encodedFiles = files.map((file) => {
    if (!isSafeTextPath(file.path))
      throw new Error(`安全でないテキストファイルパスです: ${file.path}`)
    ...
    if (content.length > MAX_TEXT_FILE_SIZE)
      throw new Error(`テキストファイルが2 MiBを超えています: ${file.path}`)
    textTotalSize += content.length
    if (textTotalSize > MAX_TEXT_TOTAL_SIZE)
      throw new Error('テキストファイルの合計が8 MiBを超えています')
