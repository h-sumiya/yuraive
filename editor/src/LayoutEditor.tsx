...
export function LayoutEditor({
  ...
  useEffect(() => {
    ...
    editor.dispatch(
      setDiagnostics(
        ...
        issues.map((issue) => ({
          from: 0,
          to: Math.min(1, editor.state.doc.length),
  ...
  return (
    <section className={`layout-editor-pane ${showPreview ? 'preview-open' : ''}`}>
      <header className="script-toolbar layout-toolbar">
        <div className="script-breadcrumb">
          <span>workspace</span>
          <b>/</b>
          <strong>{layout.path}</strong>
          {layout.dirty && <i>未保存</i>}
        </div>
        <div className="script-actions">
          <button
            className={`tool-button ${showPreview ? 'active' : ''}`}
            onClick={() => setShowPreview(!showPreview)}
          >
            プレビュー
          </button>
          <button className="toolbar-button" disabled={!layout.dirty} onClick={onSave}>
            保存
          </button>
        </div>
      </header>
      ...
      <footer
        className={`script-status ${issues.some((issue) => issue.severity === 'error') ? 'error' : ''}`}
      >
        <div>
          <span>
            {issues.length
              ? `${issues.length}件のレイアウト診断`
              : 'Yuraive Layout · HTML/CSS · UTF-8'}
          </span>
        </div>
        <div>
          <span>{layoutSlotNames(layout.content).length} slots</span>
          <span>
...
export function LayoutInspector({ layout }: { layout: LayoutDocument }) {
  ...
  return (
    <aside className="inspector layout-inspector">
      <div className="panel-title">
        <span>レイアウト</span>
        <small>HTML / CSS Grid</small>
      </div>
      <div className="inspector-scroll">
        <div className="script-file-card layout-file-card">
          <span>▦</span>
          <div>
            <strong>{layout.name}</strong>
            <small>{layout.path}</small>
          </div>
        </div>
        {issues.length > 0 && (
        ...
        <section className="script-side-section">
          ...
          {slots.map((slot, index) => (
            <div className="layout-slot-row" key={`${slot}-${index}`}>
              <code>{slot || '(default)'}</code>
              <span>{slot ? `targetSlot: "${slot}"` : 'targetSlot省略時'}</span>
            </div>
          ))}
        </section>
        <section className="script-side-section script-api">
          <h3>使用できる要素</h3>
          {LAYOUT_ELEMENTS.map((name) => (
            <code key={name}>&lt;{name}&gt;</code>
          ))}
          <span>属性: class / id / name / style / role / aria-label</span>
        </section>
        <section className="script-side-section script-api">
          <h3>Canvas変数</h3>
          {LAYOUT_VARIABLES.map((name) => (
            <code key={name}>var({name})</code>
          ))}
        </section>
        <section className="script-side-section layout-property-list">
          <h3>対応CSS</h3>
          <div>
            {LAYOUT_CSS_PROPERTIES.map((name) => (
              <code key={name}>{name}</code>
            ))}
          </div>
          <p>
            Grid、絶対配置、calc()/min()/max()/clamp()、px/%/fr/cqw/cqhを利用できます。未対応要素・属性はプレイヤーが除去します。
          </p>
        </section>
      </div>
    </aside>
