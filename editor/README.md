...
`editor.yuraive.com` をCustom DomainにしたWorkers Static Assetsとして公開します。
`yuraive.com` のゾーンを管理できるCloudflareアカウントでログインして実行してください。
...
公開せずにWorkerの構成と生成物だけを検証する場合は `npm run deploy:dry-run`、
ローカルのWorker環境で確認する場合は `npm run cf:dev` を使います。

初回デプロイ時にCustom Domain用のDNSレコードと証明書がCloudflareによって作成されます。
同名のCNAMEレコードが既にある場合は、先にCloudflare DNSから削除してください。
