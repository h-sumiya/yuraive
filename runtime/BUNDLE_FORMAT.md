# Yuraive Player Bundle v1

Yuraive Player Bundleは、配布用グラフと作者が作成したテキストファイルを1つにまとめる
バイナリ形式です。拡張子は`.yuraive`です。編集可能な元データである`.yuraive.json`は
引き続きサポートし、同じフォルダに同名の両形式がある場合はプレイヤーが`.yuraive`を優先します。

`story.yuraive.json`に対応するバンドル名は`story.yuraive`です。

## 1. 格納対象

バンドルには次のデータだけを格納します。

- `editor`という名前の全フィールドを除去した、UTF-8 JSONのYuraive v1グラフ
- コンテンツルート以下のUTF-8 `.star`ファイル
- コンテンツルート以下のUTF-8 `.yuraive-layout.html`ファイル

音声、動画、画像、字幕、ボタン背景画像は格納しません。これらは従来どおり、`.yuraive`の
