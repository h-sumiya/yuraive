...
public sealed record YuraiveLayoutIssue(ValidationSeverity Severity, string Message);

public static partial class YuraiveLayout
{
    ...
    public static IReadOnlyList<YuraiveLayoutIssue> Validate(string source)
    {
        var slots = SlotIdentifiers(source);
        var issues = new List<YuraiveLayoutIssue>();
    ...
    public static string BuildDocument(string source)
    {
        ...
        return $$"""
            ...
            <meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'; img-src data: https://yuraive.local">
            <style>
            html,body,#yuraive-layout-root{width:100%;height:100%;margin:0;overflow:hidden;background:transparent}
            html{container:yuraive-canvas / size}
            *,*::before,*::after{box-sizing:border-box}
            .yuraive-button{all:unset;box-sizing:border-box;-webkit-tap-highlight-color:transparent;cursor:pointer}
            </style><style>{{css}}</style></head><body><div id="yuraive-layout-root">{{body}}</div></body></html>
            """;
    }
