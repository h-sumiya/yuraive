...
public sealed class GraphPlaybackEngine : IAsyncDisposable
{
    ...
    private async Task<IReadOnlyList<RenderedButton>> RenderButtonsAsync(WmgNode node, CancellationToken cancellationToken)
    {
        ...
        foreach (var id in node.Buttons)
        {
            ...
            if (button.Render is not null)
            {
                try
                {
                    ...
                    result = JsonSerializer.Deserialize<ButtonRenderResult>(value.GetRawText(), WmgJson.Options);
                }
                catch (Exception error) when (error is not OperationCanceledException)
                {
    ...
    private JsonElement ContextJson(JsonObject trigger)
    {
        ...
        var value = new JsonObject
        {
            ...
            ["history"] = JsonSerializer.SerializeToNode(_history, WmgJson.Options),
            ["current"] = current,
            ["trigger"] = trigger.DeepClone(),
        };
        return JsonSerializer.SerializeToElement(value, WmgJson.Options);
    }
    ...
    private PlayerControlSettings ResolvedControls(WmgNode? node = null)
    {
    ...
    private async Task LoadLayoutAsync(WmgNode node, CancellationToken cancellationToken)
    {
    ...
    private static string? SceneLabel(WmgNode? node)
    {
