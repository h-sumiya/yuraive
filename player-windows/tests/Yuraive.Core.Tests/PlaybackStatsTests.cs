...
public sealed class PlaybackStatsTests
{
    ...
    public async Task StatsScriptReceivesSessionsAndProducesValidatedDisplayAndShareData()
    {
        ...
        await File.WriteAllTextAsync(System.IO.Path.Combine(content, "story.yuraive.json"), YuraiveJson.Serialize(graph));
        ...
        Assert.Contains("@yuraive", item.Share?.ComposedText());
    }
