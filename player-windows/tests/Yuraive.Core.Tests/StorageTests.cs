...
public sealed class StorageTests
{
    ...
    public async Task SnapshotRoundTripsAndIgnoresCorruption()
    {
        ...
        {
            GraphRef = new() { RootUri = temporary.Path, RootName = "test", RelativePath = "story.yuraive.json" },
            RunId = "run",
    ...
    public async Task LibraryScansNestedContentAndKeepsAssetsInsideTheGrantedRoot()
    {
        ...
        Assert.Equal("Yuraive", preview.Author);
    ...
    public async Task LibraryPrefersBundleAndReadsEmbeddedTextAssets()
    {
        ...
        await File.WriteAllTextAsync(System.IO.Path.Combine(root, "story.yuraive.json"),
            """{"version":1,"metadata":{"displayName":"JSON"},"nodes":{"start":{"type":"media","start":true,"terminal":true}},"buttons":{},"playerControls":{}}""");
        var bundledJson = """{"version":1,"metadata":{"displayName":"Bundle"},"playbackStats":{"path":"route.star"},"nodes":{"start":{"type":"media","start":true,"terminal":true}},"buttons":{},"playerControls":{"default":{"layout":"default.yuraive-layout.html"}},"globalPlayerControl":"default"}""";
        var bundlePath = System.IO.Path.Combine(root, "story.yuraive");
        await File.WriteAllBytesAsync(bundlePath, EncodeBundle(bundledJson,
            ("route.star", "def render_stats(ctx):\n    return None\n", 1),
            ("default.yuraive-layout.html", "<style></style><slot></slot>", 2)));
        var library = new DocumentLibrary(state);
        var imported = await library.ImportBundleAsync(bundlePath);

        Assert.Equal(root, Assert.Single(library.Roots).Uri);
        Assert.Equal("story.yuraive", imported.Ref.FileName);

        var scanned = Assert.Single(await library.ScanAllAsync());
        var preview = Assert.Single(scanned.Directory!.Graphs);
        Assert.Equal("story.yuraive", preview.Ref.FileName);
        Assert.Equal($"{root}::story.yuraive.json", preview.Ref.GraphId);
