...
- .NET SDK 10 (リポジトリの `global.json` で 10.0.301 を選択)
- Rust stable の `x86_64-pc-windows-msvc` ツールチェーン
- Visual Studio Build Tools の Desktop development with C++
- WebView2 Runtime

.NET ランタイムはアプリに同梱し、Windows App SDK ランタイムはフレームワーク依存配置です。開発機に 2.2 系ランタイムがない場合は Windows App SDK の公式インストーラーを入れてください。
...
- 設定からライブラリ共有を有効にすると、証明書ピン留めと共有シークレットで認証したQUIC直接接続をAndroidへ提供します。Cloudflareは接続候補の交換だけに使い、ファイルデータは中継しません。
